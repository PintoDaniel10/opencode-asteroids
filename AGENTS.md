...
- Existe un power-up **"triple shot"**: pickup flotante (`PowerUp` con `type === 'triple'`, tridente cian `TRIPLE_COLOR` con wrap de bordes) que al tocarlo la nave dispara 3 balas en abanico desde el morro (`angle ± TRIPLE_SPREAD`) durante `TRIPLE_DURATION` segundos. `tryShoot()` ramifica por `tripleTimer > 0`. Convive con el de velocidad: ambos pueden apilarse. Mismo spawn que el de velocidad (`POWERUP_CHANCE` al destruir `size >= 2`), reparto 50/50 (`'boost'` vs `'triple'`). Se pierde al morir y al avanzar de nivel (`ship.reset()` pone `tripleTimer = 0`). El HUD muestra `TRIPLE  {t}s` en cian, apilado bajo `VELOCIDAD x2` si ambos están activos.
